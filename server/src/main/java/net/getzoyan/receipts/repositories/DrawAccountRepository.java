...
public interface DrawAccountRepository extends JpaRepository<DrawAccount, Long> {
    ...
    @Query(value = "SELECT da.* FROM receipts.draw_account da LEFT JOIN receipts.receipts r ON r.draw_account = da.id GROUP BY r.draw_account ORDER BY COUNT(r.draw_account) DESC", nativeQuery = true)
